...
public final class ViewfinderView extends View {
	...
	private int i = 0;// 添加的
	private boolean upper; // 扫描线是否在上升中
	...
	public void onDraw(Canvas canvas) {
		...
		if (resultBitmap != null) {
			...
			// 左上角
			canvas.drawRect(frame.left, frame.top, frame.left + 40, frame.top + 10, paint);
			canvas.drawRect(frame.left, frame.top, frame.left + 10, frame.top + 40, paint);
			// 右上角
			canvas.drawRect(frame.right - 40, frame.top, frame.right, frame.top + 10, paint);
			canvas.drawRect(frame.right - 10, frame.top, frame.right, frame.top + 40, paint);
			// 左下角
			...
			// 右下角
			canvas.drawRect(frame.right - 40, frame.bottom - 10, frame.right, frame.bottom, paint);
			canvas.drawRect(frame.right - 10, frame.bottom - 40, frame.right, frame.bottom, paint);
			...
			if (!upper && (i += 6) < frame.bottom - frame.top) {
				/* 以下为用渐变线条作为扫描线 */
				// 渐变图为矩形
